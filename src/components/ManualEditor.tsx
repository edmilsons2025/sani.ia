...
const renderers: { [key in SectionContent['type']]: FC<SectionRendererProps> } = {
                ...
                <thead className="bg-gray-50">
                    <tr>
                        {section.headers.map((header: string, index: number) => (
                            <th key={index} className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{header}</th>
                        ))}
                    </tr>
                </thead>
                    ...
                    {section.rows.map((row: string[], rowIndex: number) => (
                        <tr key={rowIndex}>
                            {row.map((cell: string, cellIndex: number) => (
                                <td key={cellIndex} className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{cell}</td>
                            ))}
                        </tr>
                    ))}
                </tbody>
  ...
  list: ({ section }: SectionRendererProps) => {
    ...
    return (
        <div className="mt-3">
            {section.title && (<h4 className="font-semibold text-gray-700">{section.title}</h4>)}
  ...
  alert: ({ section }: SectionRendererProps) => {
    ...
    return (
        <div className={`mt-4 p-4 border-l-4 rounded-r-lg ${colors[section.level]}`}>
    ...
    return (
      <details className="group border border-gray-200 rounded-lg p-4 mt-4 bg-gray-50" open>
        <summary className="font-semibold text-lg text-gray-800 cursor-pointer flex justify-between items-center list-none">
          {section.title}
          <svg className="w-5 h-5 ml-4 text-cyan-600 transform group-open:rotate-180 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4 sm:p-8 font-sans antialiased">
      <div className="w-full max-w-6xl">
        <header className="bg-white shadow-lg rounded-xl p-6 mb-6 border-l-4 border-cyan-500">
            <h1 className="text-2xl font-extrabold text-cyan-700">Gerador de Manuais com IA</h1>
            <p className="text-gray-600 mt-2">
                Esta ferramenta estrutura documentação técnica em manuais interativos. Para começar, siga os passos na área de geração abaixo.
            </p>
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 mb-20">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right text-sm text-gray-500">
                        ...
                        {manualData.revision && <div>Revisão: {manualData.revision} | Data: {manualData.date}</div>}
                    </div>
                </div>
